...
try
 {
   $response = Invoke-WebRequest -Uri https://github.com/Louisjreeves/SetupWacInstall/raw/main/SetupWacInstall.zip -OutFile $Mydownloads\SetupWacInstall.zip
 ...
 } catch 
 {
    ...
    Expand-Archive -Path $mydownloads\SetupWacInstall.zip -DestinationPath $mydownloads\SetupWacInstall -Force
 
 
 $activedirectory= "C:\Users\*\Downloads\SetupWacInstall\SetupWacInstall"
 cd c:\
