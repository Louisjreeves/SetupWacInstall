...
try
 {
   $response = Invoke-WebRequest -Uri https://github.com/Louisjreeves/SetupWacInstall/raw/main/OfflineWacInstall.zip -OutFile $Mydownloads\OfflineWacInstall.zip
 ...
 } catch 
 {
    ...
    Expand-Archive -Path $mydownloads\OfflineWacInstall.zip -DestinationPath $mydownloads\OfflineWacInstall -Force
 
 
 $activedirectory= "C:\Users\$myuser\Downloads\OfflineWacInstall"
 cd c:\
 set-location $activedirectory
  .\offlineWacInstall.ps1
